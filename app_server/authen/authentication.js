  ...
  db.connect()
    ...
    .then(users => {
      if (Array.isArray(users) && users.length == 0) {
        ...
        sendJsonResponse(res, 400, {err: true, msg: 'Email existed already'})
        return;
      }
    })
    .then(results => {
  ...
  db.connect()
    ...
    .then(users => {
      if (!users || users.length == 0) {
        sendJsonResponse(res, 400, {err: true, msg: 'Incorrect email'});
        return;
      }
      let user = users[0];
      let hash = crypto.pbkdf2Sync(password, user.salt, 1000, 64).toString('hex');
      if (user.password != hash) {
        sendJsonResponse(res, 400, {err: true, msg: 'Incorrect password'});
        return;
      }
